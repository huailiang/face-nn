...
//
half3 ApplyLut2D(TEXTURE2D_ARGS(tex), half3 uvw, half4 scaleOffset)
{
    ...
    //uvw.xyz = lerp(
    //    SAMPLE_TEXTURE2D(tex, uvw.xy).rgb,
    //    SAMPLE_TEXTURE2D(tex, uvw.xy + half2(scaleOffset.y, 0.0)).rgb,
    //    uvw.z - shift
    //);
	uvw.xyz = lerp(
		SAMPLE_TEXTURE2D(tex, uvw.xy).rgb,
...
//
float3 YrgbCurve(float3 c, TEXTURE2D_ARGS(curveTex))
{
    const float kHalfPixel = (1.0 / 128.0) / 2.0;

    // Y (master)
    c += kHalfPixel.xxx;
    float mr = SAMPLE_TEXTURE2D(curveTex, float2(c.r, 0.75)).a;
    float mg = SAMPLE_TEXTURE2D(curveTex, float2(c.g, 0.75)).a;
    float mb = SAMPLE_TEXTURE2D(curveTex, float2(c.b, 0.75)).a;
    c = saturate(float3(mr, mg, mb));

    // RGB
    c += kHalfPixel.xxx;
    float r = SAMPLE_TEXTURE2D(curveTex, float2(c.r, 0.75)).r;
    float g = SAMPLE_TEXTURE2D(curveTex, float2(c.g, 0.75)).g;
    float b = SAMPLE_TEXTURE2D(curveTex, float2(c.b, 0.75)).b;
