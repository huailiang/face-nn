...
namespace CFEngine.Editor
{
    ...
    public sealed class TrackballDecorator : AttributeDecorator
    {
        ...
        {
            var param = serializedParameterOverride as SerializedParameterOverride;
            var property = param.value;

            var overrideState = param.overrideState;
            ...
            using (new EditorGUILayout.VerticalScope())
            {
                using (new EditorGUI.DisabledScope(!overrideState.boolValue))
                    DrawWheel(ref value, overrideState.boolValue, (CFTrackballAttribute)attribute);
        ...
        void DrawWheel(ref Vector4 value, bool overrideState, CFTrackballAttribute attr)
        {
            ...
            if (attr.mode == CFTrackballAttribute.Mode.None)
                return;
            ...
            switch (attr.mode)
            {
                case CFTrackballAttribute.Mode.Lift:
                    displayValue = ColorUtilities.ColorToLift(value);
                    break;
                case CFTrackballAttribute.Mode.Gamma:
                    displayValue = ColorUtilities.ColorToInverseGamma(value);
                    break;
                case CFTrackballAttribute.Mode.Gain:
